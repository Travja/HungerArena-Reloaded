...
public class CM {
	...
	public static void reloadOptions() {
		if (optionsFile == null) {
			optionsFile = new File(Main.self.getDataFolder(), "CmdsAndBlocks.yml");
		}
		options = YamlConfiguration.loadConfiguration(optionsFile);

		try{
			Reader defConfigStream = new InputStreamReader(Main.self.getResource("CmdsAndBlocks.yml"), "UTF8");
			if (defConfigStream != null) {
				YamlConfiguration defConfig = YamlConfiguration.loadConfiguration(defConfigStream);
